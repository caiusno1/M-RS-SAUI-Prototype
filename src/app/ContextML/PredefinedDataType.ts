
/** Generated Code
* emf ecore to javascript
* generated at 2019-04-24T18:33:50.201+02:00
* (C) XSLT ecore2ts - Kai Biermeier
**/
import { CustomContextPropertyDatatype } from './CustomContextPropertyDatatype';
import { EString } from './EString';

export class PredefinedDataType extends CustomContextPropertyDatatype{
  public type = 'PredefinedDataType';
  public eClass: string;
  public $ref: string;
  
  // Type: ecore:EDataType http://www.eclipse.org/emf/2002/Ecore#//EString
  public valueType: EString;

}
    